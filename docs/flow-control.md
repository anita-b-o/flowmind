...
Graph v2 supports controlled `FOR_EACH` nodes with explicit `for_each_body` and `for_each_done` edges. The body is a single-entry/single-exit acyclic region executed sequentially for each array item; repetition is owned by the runtime and never represented by a graph back-edge. Body expressions receive iteration-local `item` and `index`, while execution/workflow variables remain shared. Parallel mode, nested loops, WHILE, break/continue, explicit join nodes and subworkflows remain out of scope.
