...
import { canManageNotificationRules } from "../../features/auth/rbac";
import { apiClient } from "../../lib/api-client";
import { useNotificationRuleMutations, useNotificationRules, useNotifications } from "../../features/notifications/hooks";
import type { NotificationEventType, NotificationRule, NotificationStatus } from "../../features/notifications/types";

const EVENTS: NotificationEventType[] = ["APPROVAL_REQUESTED", "APPROVAL_APPROVED", "APPROVAL_REJECTED", "APPROVAL_EXPIRED", "EXECUTION_COMPLETED", "EXECUTION_FAILED", "EVENT_TRIGGER_FAILED", "EVENT_CHAIN_DEPTH_EXCEEDED"];
const TEMPLATES: Record<NotificationEventType, string> = { APPROVAL_REQUESTED: "approval.requested", APPROVAL_APPROVED: "approval.approved", APPROVAL_REJECTED: "approval.rejected", APPROVAL_EXPIRED: "approval.expired", EXECUTION_COMPLETED: "workflow.completed", EXECUTION_FAILED: "workflow.failed", EVENT_TRIGGER_FAILED: "event-trigger.failed", EVENT_CHAIN_DEPTH_EXCEEDED: "event-chain.depth-exceeded" };
const LABELS: Partial<Record<NotificationEventType, string>> = { EXECUTION_COMPLETED: "WORKFLOW_COMPLETED", EXECUTION_FAILED: "WORKFLOW_FAILED" };

export default function NotificationsPage() {
  const [tab, setTab] = useState<"rules" | "deliveries">("rules"); const { organizations, activeOrganizationId } = useAuth(); const role = organizations.find((item) => item.id === activeOrganizationId)?.role; const canManage = canManageNotificationRules(role);
  return <RequireAuth><main className="content stack"><h1>Notifications</h1><div><button onClick={() => setTab("rules")} disabled={tab === "rules"}>Rules</button> <button onClick={() => setTab("deliveries")} disabled={tab === "deliveries"}>Deliveries</button></div>{tab === "rules" ? <Rules canManage={canManage} /> : <Deliveries />}</main></RequireAuth>;
}
...
function RuleForm({ initial, connections, busy, onCancel, onSave }: { initial: NotificationRule | null; connections: Array<{ id: string; name: string }>; busy: boolean; onCancel: () => void; onSave: (dto: Record<string, unknown>) => void }) {
  const [eventType, setEventType] = useState<NotificationEventType>(initial?.eventType ?? "APPROVAL_REQUESTED"); const initialRecipients = initial?.recipientConfigJson; const [kind, setKind] = useState<"EMAILS" | "APPROVAL_ROLES">(initialRecipients?.kind ?? "EMAILS"); const [emails, setEmails] = useState(initialRecipients?.kind === "EMAILS" ? initialRecipients.emails.join(", ") : ""); const [roles, setRoles] = useState<string[]>(initialRecipients?.kind === "APPROVAL_ROLES" ? initialRecipients.roles : ["owner", "admin"]); const [connectionId, setConnectionId] = useState(initial?.connectionId ?? connections[0]?.id ?? ""); const [enabled, setEnabled] = useState(initial?.enabled ?? true); const approval = eventType.startsWith("APPROVAL_");
