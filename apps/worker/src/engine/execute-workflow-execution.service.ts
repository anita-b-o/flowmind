...
export class ExecuteWorkflowExecutionService {
  ...
  async execute(input: Input): Promise<ExecuteWorkflowRunResult> {
    ...
    const parent = await this.prisma.execution.findFirstOrThrow({ where: { id: input.executionId, organizationId: input.organizationId }, select: { id: true, workflowId: true, rootExecutionId: true, depth: true, executionMode: true } });
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.execution.create({ data: { id: childId, organizationId: input.organizationId, workflowId: config.workflowId, workflowVersionId: target.id, parentExecutionId: parent.id, parentStepExecutionId: input.stepExecution.id, rootExecutionId: rootId, depth: parent.depth + 1, correlationId, status: ExecutionStatus.Queued, executionMode: parent.executionMode as ExecutionMode, inputJson: json({ trigger: { input: childInput }, metadata: { subworkflow: true, depth: parent.depth + 1 } }), contextJson: json({ trigger: { input: childInput }, steps: {}, metadata: { subworkflow: true } }) } });
    });
