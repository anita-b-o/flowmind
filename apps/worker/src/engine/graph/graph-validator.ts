...
export type RuntimeGraphEdge = {
  from: string;
  to: string;
  kind: "next" | "if_true" | "if_false" | "switch_case" | "switch_default" | "for_each_body" | "for_each_done" | "try_body" | "try_catch" | "try_finally" | "try_done" | "approval_approved" | "approval_rejected" | "approval_expired";
  label?: string;
  caseKey?: string;
};
...
export function parseRuntimeGraph(definition: unknown): RuntimeGraph | null {
  ...
  const edges = Array.isArray(graph.edges)
    ? graph.edges
        ...
        .filter((edge) => edge.from && edge.to && ["next", "if_true", "if_false", "switch_case", "switch_default", "for_each_body", "for_each_done", "try_body", "try_catch", "try_finally", "try_done", "approval_approved", "approval_rejected", "approval_expired"].includes(edge.kind))
    : [];
