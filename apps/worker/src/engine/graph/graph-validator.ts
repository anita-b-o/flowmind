...
export type RuntimeGraphEdge = {
  from: string;
  to: string;
  kind: "next" | "if_true" | "if_false" | "switch_case" | "switch_default";
  label?: string;
  caseKey?: string;
};
...
export function parseRuntimeGraph(definition: unknown): RuntimeGraph | null {
  ...
  const edges = Array.isArray(graph.edges)
    ? graph.edges
        ...
        .filter((edge) => edge.from && edge.to && ["next", "if_true", "if_false", "switch_case", "switch_default"].includes(edge.kind))
    : [];
