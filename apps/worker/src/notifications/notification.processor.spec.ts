...
function fixture(provider: { send: jest.Mock; requiresConnection?: boolean }) {
  ...
  const processor = new NotificationProcessor(
    ...
    provider as any,
    { isShuttingDown: () => false } as any
  );
  return { processor, requestUpdates, deliveryUpdates, tx, connections };
}
describe("NotificationProcessor", () => {
  ...
  it("runs the embedded fake provider without resolving SMTP credentials", async () => {
    const provider = { requiresConnection: false, send: jest.fn(async () => ({ messageId: "demo-message" })) };
    const test = fixture(provider);
    await test.processor.deliver("request-1");
    expect(test.connections.resolveSmtp).not.toHaveBeenCalled();
    expect(provider.send).toHaveBeenCalledWith(expect.objectContaining({ connection: undefined }));
    expect(test.requestUpdates).toContainEqual(expect.objectContaining({ status: "SENT" }));
  });
});
