...
export function buildVariableCatalog(input: { steps?: WorkflowStepLike[]; includeMetadata?: boolean } = {}): VariableCatalogEntry[] {
  const entries: VariableCatalogEntry[] = [
    ...
    entry("trigger.event", "object", "Internal event", "Safe immutable internal event envelope", "trigger"),
    entry("trigger.event.type", "string", "Internal event type", undefined, "trigger"),
    entry("trigger.event.data", "object", "Internal event data", "Sanitized event-specific data", "trigger"),
